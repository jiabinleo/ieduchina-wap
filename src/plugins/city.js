function address(province, city) {
    var _province = $("#".concat(province));
    var _city = $("#".concat(city));
    var cityplaceholder = _city.html();
    layui.use(function () {
        var form = layui.form;
        for (var i = 0; i < provinceList.length; i++) {
            _province.append("<option value=\"".concat(provinceList[i].name, "\">").concat(provinceList[i].name, "</option>"));
        }
        form.render("select");
        form.on("select(" + province + ")", function (item) {
            var prov = item.value;
            _city.empty();
            _city.append(cityplaceholder);
            for (var j = 0; j < provinceList.length; j++) {
                if (provinceList[j].name == prov) {
                    for (var k = 0; k < provinceList[j].cityList.length; k++) {
                        _city.append("<option value=\"".concat(provinceList[j].cityList[k].name, "\">").concat(provinceList[j].cityList[k].name, "</option>"));
                    }
                    form.render("select");
                }
            }
